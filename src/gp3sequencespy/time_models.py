...
def fit_time_varying_sequence_model(
    ...
    try:
        ...
        raise ValidationError(f"Time-varying model fitting failed: {e}") from e
...
def predict_time_varying_sequence_model(
    ...
    try:
        X = patsy.build_design_matrices([model.design_info], grid, return_type="dataframe")[0]
    except Exception as e:
        raise ValidationError(f"Prediction design construction failed: {e}") from e
    params = np.asarray(model.model.params)
    cov = np.asarray(model.model.cov_params())
    eta = np.asarray(X) @ params
    se = np.sqrt(np.maximum(np.einsum("ij,jk,ik->i", np.asarray(X), cov, np.asarray(X)), 0))
    z = norm.ppf(1 - (1 - level) / 2)
    return pd.DataFrame(
        {
            "time": grid.time.to_numpy(),
            "group": grid.group.to_numpy(),
            "estimate": expit(eta),
...
def summarise_time_varying_sequence_model(model: TimeVaryingSequenceModel) -> dict[str, Any]:
    ...
    coef = pd.DataFrame(
        {
            "coefficient": model.model.params,
            "std_error": model.model.bse,
            "z": model.model.tvalues,
            "p_value": model.model.pvalues,
        }
    ).reset_index(names="term")
    smooth = coef.loc[coef.term.str.contains("bs\\(time", regex=True)].reset_index(drop=True)
    param = coef.loc[~coef.term.str.contains("bs\\(time", regex=True)].reset_index(drop=True)
    md = pd.DataFrame(
        [
            {
                "outcome": model.outcome,
                ...
                "k": model.k,
                "deviance_explained": float(1 - model.model.deviance / model.model.null_deviance)
    ...
    return {
        "metadata": md,
        "parametric_terms": param,
        "smooth_terms": smooth,
        "converged": bool(getattr(model.model, "converged", True)),
        "method": model.method,
    }
