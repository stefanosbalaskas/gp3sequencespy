...
def _audit_frame(issues: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    df = df.sort_values(["__sev", "__seq", "__row", "issue_code"], kind="stable").drop(
        columns=["__sev", "__seq", "__row"]
    )
...
def audit_sequence_data(
    ...
    mapped_core = [
        x for x in [sequence_id_col, order_col, state_col, duration_col] if x is not None
    ]
        ...
        if col not in data.columns:
            issues.append(
                _issue(
                    ...
                    severity="error",
                    value=col,
                    message=f"The mapped {role} column is absent from `data`.",
    ...
    for idx in np.flatnonzero(seq_missing.to_numpy()):
        issues.append(
            _issue(
                ...
                severity="error",
                value=_value_text(seq.iloc[idx]),
    ...
    for idx in np.flatnonzero(state_missing.to_numpy()):
        ...
        issues.append(
            _issue(
                ...
                severity="error",
                value=_value_text(state.iloc[idx]),
    ...
    if valid_order:
        ...
        for idx in np.flatnonzero(order_missing.to_numpy()):
            issues.append(
                _issue(
                    sequence_id=seq_text.iloc[idx],
                    row=idx + 1,
                    column=order_col,
                    issue_code="missing_sequence_order",
                    severity="error",
                    value=_value_text(order.iloc[idx]),
    ...
    if valid_order:
        ...
        if len(valid_order_pos):
            keys = [
                (str(seq_text.iloc[i]), _format_order_key(order.iloc[i])) for i in valid_order_pos
            ]
            ...
            for pos, k in zip(valid_order_pos, keys, strict=True):
                if counts[k] > 1:
                    issues.append(
                        _issue(
                            sequence_id=seq_text.iloc[pos],
                            row=pos + 1,
                            column=order_col,
                            issue_code="duplicated_position",
                            severity="error",
                            value=_value_text(order.iloc[pos]),
                            message="The sequence contains a duplicated position.",
                            action="Correct the position or apply an explicit first/last policy.",
                        )
                    )
            order_groups: dict[str, list[int]] = {}
            for pos in valid_order_pos:
                order_groups.setdefault(str(seq_text.iloc[pos]), []).append(int(pos))
            eps = np.sqrt(np.finfo(float).eps)
            for sid, positions in order_groups.items():
                ...
                if len(unique_order) > 1 and np.all(
                    np.abs(unique_order - np.round(unique_order)) < eps
                ):
                    gaps = np.flatnonzero(np.diff(unique_order) > 1)
                    if len(gaps):
                        gap_text = ", ".join(
                            f"{unique_order[g]:g}->{unique_order[g + 1]:g}" for g in gaps
                        )
                        issues.append(
                            _issue(
                                sequence_id=sid,
                ...
                for pos in ordered_positions:
                    cur_missing = bool(state_missing.iloc[pos])
                    cur = str(state_text.iloc[pos])
                    if not cur_missing and not prev_missing and cur == prev:
                        issues.append(
                            _issue(
                                ...
                                severity="review",
                                value=_value_text(state.iloc[pos]),
    ...
    if len(valid_seq_pos):
        ...
        for pos in valid_seq_pos:
            sequence_groups.setdefault(str(seq_text.iloc[pos]), []).append(int(pos))
        for sid, positions in sequence_groups.items():
            ...
            if len(usable) == 1:
                ...
                issues.append(
                    _issue(
                        ...
                        severity="review",
                        value=_value_text(state.iloc[i]),
    ...
    if duration_col is not None and duration_col in data.columns:
        ...
        if not valid_duration:
            ...
            miss = dur.isna()
            finite = pd.Series(np.isfinite(vals), index=data.index)
            for idx in np.flatnonzero(miss.to_numpy()):
                issues.append(
                    _issue(
                        ...
                        severity="review",
                        value=_value_text(dur.iloc[idx]),
    ...
    for col in [x for x in metadata if x in data.columns]:
        ...
        if _is_list_column(vals):
            ...
            continue
        if not len(valid_seq_pos):
            continue
        metadata_groups: dict[str, list[int]] = {}
        for pos in valid_seq_pos:
            metadata_groups.setdefault(str(seq_text.iloc[pos]), []).append(int(pos))
        for sid, positions in metadata_groups.items():
            ...
            if len(unique) > 1:
                issues.append(
                    _issue(
                        ...
                        severity="error",
                        value=_value_text(unique),
                        message=f"Metadata column `{col}` varies within the sequence.",
...
def validate_sequence_data(
    ...
    audit = audit_sequence_data(
        data, sequence_id_col, order_col, state_col, duration_col, metadata, expected_states
    )
    ...
    return ValidationResult(
        ...
        counts["info"],
        audit,
        _mapping(sequence_id_col, order_col, state_col, duration_col, metadata),
        len(data),
        nseq,
        levels,
    )
...
def _stage_audit(audit: pd.DataFrame, stage: str) -> pd.DataFrame:
    if len(audit) == 0:
        return pd.concat(
            [pd.DataFrame({"stage": pd.Series(dtype="object")}), _empty_audit()], axis=1
        )
    out = audit.copy()
...
def prepare_sequence_data(
    ...
    if initial.audit["issue_code"].isin(fatal).any():
        return PrepareResult(
            None, input_audit, _empty_decisions(), initial.mapping, "fail", len(data), 0, []
        )
    ...
    def decision(step: str, policy: str, affected: int, details: str) -> None:
        decisions.append(
            {"step": step, "policy": policy, "affected_rows": int(affected), "details": details}
        )

    miss = _missing_mask(working[state_col])
    if missing_state_policy == "drop":
        affected = int(miss.sum())
        working = working.loc[~miss].copy()
        decision(
            "missing_states", "drop", affected, "Rows with missing or blank states were removed."
        )
    else:
        decision(
            "missing_states",
            "error",
            int(miss.sum()),
            "Missing states were retained as unresolved errors.",
        )

    unknown = pd.Series(False, index=working.index)
    if expected_states is not None and len(working):
        expected = {str(x) for x in expected_states}
        unknown = (~_missing_mask(working[state_col])) & (
            ~working[state_col].astype(str).isin(expected)
        )
    if unknown_state_policy == "drop":
        affected = int(unknown.sum())
        working = working.loc[~unknown].copy()
        decision(
            "unknown_states", "drop", affected, "States absent from `expected_states` were removed."
        )
    else:
        details = "Unknown states were " + (
            "retained as unresolved errors."
            if unknown_state_policy == "error"
            else "preserved for review."
        )
    ...
    if duration_col is not None and len(working):
        d = pd.to_numeric(working[duration_col], errors="coerce").astype(float)
        zero = working[duration_col].notna() & np.isfinite(d) & (d == 0)
    if zero_duration_policy == "drop":
        affected = int(zero.sum())
        working = working.loc[~zero].copy()
        decision("zero_durations", "drop", affected, "Rows with zero duration were removed.")
    else:
        details = "Zero-duration rows were " + (
            "retained as unresolved errors." if zero_duration_policy == "error" else "preserved."
        )
    ...
    keys = (
        list(
            zip(
                ...
                working[order_col].map(_format_order_key),
                strict=True,
            )
        )
        if len(working)
    ...
    if duplicate_position_policy in {"first", "last"}:
        ...
        if keys:
            if duplicate_position_policy == "first":
                keep = ~pd.Series(keys, index=working.index).duplicated(keep="first")
            else:
                keep = ~pd.Series(keys, index=working.index).duplicated(keep="last")
        affected = int((~keep).sum())
        ...
        decision(
            ...
            f"{duplicate_position_policy} occurrence.",
        )
    else:
        s = pd.Series(keys)
        count = int(s.duplicated(False).sum()) if len(s) else 0
        decision(
            "duplicated_positions",
            "error",
            count,
            "Duplicated positions were retained as unresolved errors.",
        )

    before = working.index.tolist()
    working = working.sort_values(
        [sequence_id_col, order_col, ".gp3_original_row"], kind="stable", na_position="last"
    )
    ...
    decision(
        ...
        "deterministic_sort",
        moved,
        "Rows were ordered by sequence identifier, sequence order, and original row.",
    )

    collapsed = 0
    if repeated_state_policy == "collapse" and len(working) > 1:
        seqtxt = working[sequence_id_col].astype(str)
        statetxt = working[state_col].astype("string")
        missing = _missing_mask(working[state_col])
        same = (
            seqtxt.eq(seqtxt.shift(1))
            & ~missing
        ...
        if duration_col is not None:
            for _, idxs in working.groupby(group, sort=False).groups.items():
                ...
                if len(idxs) > 1:
                    ...
                    working.loc[idxs[0], duration_col] = (
                        np.nan if vals.isna().all() else vals.sum(skipna=True)
                    )
        keep = ~group.duplicated()
    ...
    decision(
        ...
        collapsed,
        "The first row was retained for each repeated run; available durations were summed."
        if repeated_state_policy == "collapse"
        else "Consecutive repeated states were preserved.",
    )

    removed_levels = 0
    if unused_state_levels == "drop" and isinstance(working[state_col].dtype, pd.CategoricalDtype):
        before = set(working[state_col].cat.categories)
        working[state_col] = working[state_col].cat.remove_unused_categories()
        removed_levels = len(before - set(working[state_col].cat.categories))
    decision(
        "unused_state_levels",
        unused_state_levels,
        removed_levels,
        "Unused factor levels were removed."
        if unused_state_levels == "drop"
        else "Unused factor levels were preserved.",
    )
    ...
    canonical_names = ["sequence_id", "sequence_order", "state", "original_row"] + (
        ["duration"] if duration_col else []
    )
    collisions = [c for c in extras if c in canonical_names]
    if collisions:
        raise ValidationError(
            "Unmapped source columns use reserved canonical names: " + ", ".join(collisions) + "."
        )
    canonical = pd.DataFrame(
        {
            "sequence_id": working[sequence_id_col].values,
            "sequence_order": working[order_col].values,
            "state": working[state_col].values,
            "original_row": working[".gp3_original_row"].astype(int).values,
        }
    )
    if duration_col is not None:
        canonical["duration"] = working[duration_col].values
    for col in extras:
        canonical[col] = working[col].values
    decision(
        "column_mapping",
        "canonicalise",
        len(canonical),
        "Mapped columns were standardised while unmapped columns were preserved.",
    )

    outval = validate_sequence_data(
        canonical,
        "sequence_id",
        "sequence_order",
        "state",
        "duration" if duration_col else None,
        metadata,
        expected_states,
    )
    final = outval.audit.copy()
    policy = []
    if zero_duration_policy == "error" and "duration" in canonical:
        vals = pd.to_numeric(canonical["duration"], errors="coerce")
        rows = canonical["duration"].notna() & np.isfinite(vals) & (vals == 0)
        for i in np.flatnonzero(rows.to_numpy()):
            policy.append(
                _issue(
                    sequence_id=canonical.loc[i, "sequence_id"],
                    row=int(canonical.loc[i, "original_row"]),
                    column="duration",
    ...
    if unknown_state_policy == "error" and expected_states is not None:
        expected = {str(x) for x in expected_states}
        rows = (~_missing_mask(canonical["state"])) & (
            ~canonical["state"].astype(str).isin(expected)
        )
        for i in np.flatnonzero(rows.to_numpy()):
            policy.append(
                _issue(
                    ...
                    severity="error",
                    value=_value_text(canonical.loc[i, "state"]),
    ...
    if policy:
        final = _audit_frame(
            pd.concat([final, pd.DataFrame(policy)], ignore_index=True).to_dict("records")
        )
    combined = pd.concat([input_audit, _stage_audit(final, "output")], ignore_index=True)
    decisions_df = (
        pd.DataFrame(decisions, columns=_DECISION_COLUMNS) if decisions else _empty_decisions()
    )
    final_status = _status(final)
    if isinstance(canonical["state"].dtype, pd.CategoricalDtype):
        levels = [str(x) for x in canonical["state"].cat.categories]
    else:
        levels = list(
            dict.fromkeys(
                canonical.loc[~_missing_mask(canonical["state"]), "state"].astype(str).tolist()
            )
        )
    return PrepareResult(
        None if final_status == "fail" else canonical,
        combined,
        decisions_df,
        initial.mapping,
        final_status,
