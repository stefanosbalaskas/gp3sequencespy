...
from .data import _normalize_cols, validate_sequence_data
...
def _assert_output_names(
    metadata_cols: Sequence[str] | None, reserved: Collection[str], context: str
) -> list[str]:
    cols = _normalize_cols(metadata_cols, "metadata_cols")
    collisions = [x for x in cols if x in reserved]
    if collisions:
        raise ValidationError(
            f"Metadata column names conflict with {context} output columns: "
            + ", ".join(collisions)
            + "."
        )
    return cols
...
def _summary_input(
    ...
    val = validate_sequence_data(
        data, sequence_id_col, order_col, state_col, duration_col, metadata, expected_states
    )
    if not val.valid:
        codes = list(
            dict.fromkeys(val.audit.loc[val.audit["severity"] == "error", "issue_code"].tolist())
        )
        raise ValidationError(
            "Sequence data failed validation: "
            + ", ".join(codes)
            + ". Resolve the errors or use `prepare_sequence_data()` with explicit policies "
            "before continuing."
        )
    ...
    if not pd.api.types.is_numeric_dtype(sort_seq.dtype):
        sort_seq = sort_seq.astype(str)
    working = (
        working.assign(__seq_sort=sort_seq)
        .sort_values(["__seq_sort", order_col, "__original_row"], kind="stable")
        .drop(columns="__seq_sort")
    ...
    original_levels = (
        [str(x) for x in data[state_col].cat.categories]
        if isinstance(data[state_col].dtype, pd.CategoricalDtype)
        else None
    )
    return {
        "data": canonical,
        ...
        "status": val.status,
        "mapping": val.mapping,
        "original_state_levels": original_levels,
    }
...
def encode_sequence_data(
    ...
    inp = _summary_input(
        data, sequence_id_col, order_col, state_col, duration_col, metadata_cols, expected_states
    )
    _assert_output_names(metadata_cols, ["state_index", "state_code"], "encoded")
    observed = list(dict.fromkeys(inp["data"]["state"].tolist()))
    if state_levels is None:
        resolved = (
            inp["original_state_levels"]
            if inp["original_state_levels"] is not None
        ...
        if any(x is None or not str(x).strip() for x in state_levels) or len(set(resolved)) != len(
            resolved
        ):
        ...
        if omitted:
            raise ValidationError(
                "`state_levels` omits observed states: " + ", ".join(omitted) + "."
            )
    if width is None:
        resolved_width = max(1, len(str(len(resolved))))
    else:
        if (
            isinstance(width, bool)
            or not isinstance(width, (int, np.integer, float, np.floating))
    ...
    idx = dict(zip(dictionary.state, dictionary.state_index, strict=True))
    codes = dict(zip(dictionary.state, dictionary.state_code, strict=True))
    ...
    return EncodingResult(
        ...
        inp["audit"],
        resolved,
        {"prefix": prefix, "width": resolved_width},
    )
...
def _sum_or_na(s: pd.Series) -> float:
    return (
        ...
        else float(pd.to_numeric(s, errors="coerce").sum(skipna=True))
    )
...
def _mean_or_na(s: pd.Series) -> float:
    return (
        float("nan")
        if len(s) == 0 or s.isna().all()
        else float(pd.to_numeric(s, errors="coerce").mean(skipna=True))
    )


def _safe_prop(num: float, den: float | None) -> float:
    return float("nan") if den is None or pd.isna(den) or den == 0 else float(num / den)
...
def summarise_sequence_states(
    ...
    include_duration = "duration" in df
    seq_rows = []
    for sid in ids:
        rows = df.index[df.sequence_id == sid].tolist()
        meta = _meta_values(df, rows, metadata)
        states = df.loc[rows, "state"].tolist()
        ...
        for st in state_order:
            ...
            rec = {
                ...
                **meta,
                "state": st,
                "n_observations": len(sr),
                "observation_proportion": len(sr) / n,
            }
            if include_duration:
                sd = _sum_or_na(df.loc[sr, "duration"])
                rec.update(
                    duration_sum=sd,
                    duration_proportion=_safe_prop(sd, seq_duration),
                    mean_duration=_mean_or_na(df.loc[sr, "duration"]),
                )
            seq_rows.append(rec)
    ...
    total_obs = len(df)
    total_seq = len(ids)
    total_dur = _sum_or_na(df.duration) if include_duration else None
    for st in list(dict.fromkeys(df.state.tolist())):
        sr = df.index[df.state == st].tolist()
        sids = list(dict.fromkeys(df.loc[sr, "sequence_id"].tolist()))
        rec = {
            "state": st,
            "n_sequences": len(sids),
            "sequence_proportion": len(sids) / total_seq,
            "n_observations": len(sr),
            "observation_proportion": len(sr) / total_obs,
        }
        if include_duration:
            sd = _sum_or_na(df.loc[sr, "duration"])
            rec.update(
                duration_sum=sd,
                duration_proportion=_safe_prop(sd, total_dur),
                mean_duration=_mean_or_na(df.loc[sr, "duration"]),
            )
        overall.append(rec)
    return StateSummaryResult(
        by, pd.DataFrame(overall), inp["audit"], inp["status"], inp["mapping"]
    )
...
def summarise_sequence_transitions(
    ...
    inp = _summary_input(
        data, sequence_id_col, order_col, state_col, None, metadata, expected_states
    )
    df = inp["data"]
    ids = list(dict.fromkeys(df.sequence_id.tolist()))
    by_rows = []
    ...
    for sid in ids:
        ...
        if len(rows) < 2:
            continue
        states = df.loc[rows, "state"].tolist()
        pairs = list(zip(states[:-1], states[1:], strict=True))
        if not include_self:
        ...
        total = len(pairs)
        for a, b in unique:
            count = sum(p == (a, b) for p in pairs)
            origin = sum(p[0] == a for p in pairs)
            by_rows.append(
                {
                    "sequence_id": sid,
                    **meta,
                    ...
                    "to_state": b,
                    "n_transitions": count,
                    "sequence_transition_proportion": count / total,
                    "origin_transition_proportion": count / origin,
                }
            )
    rawdf = pd.DataFrame(raw, columns=["sequence_id", "from_state", "to_state"])
    overall = []
    if len(rawdf):
        pairs = list(dict.fromkeys(zip(rawdf.from_state, rawdf.to_state, strict=True)))
        total = len(rawdf)
        total_seq = len(ids)
        for a, b in pairs:
            mask = (rawdf.from_state == a) & (rawdf.to_state == b)
            count = int(mask.sum())
            origin = int((rawdf.from_state == a).sum())
            contrib = rawdf.loc[mask, "sequence_id"].nunique()
            overall.append(
                {
                    "from_state": a,
                    "to_state": b,
                    "n_sequences": int(contrib),
                    "sequence_proportion": contrib / total_seq,
                    "n_transitions": count,
                    "transition_proportion": count / total,
                    "origin_transition_proportion": count / origin,
                }
            )
    by_columns = [
...
def format_sequence_paths(
    ...
    _assert_flag(collapse_repeats, "collapse_repeats")
    metadata = _assert_output_names(
        metadata_cols,
        ["n_observations", "n_states", "n_unique_states", "start_state", "end_state", "path"],
        "path",
    )
    inp = _summary_input(
        data, sequence_id_col, order_col, state_col, None, metadata, expected_states
    ...
    for sid in list(dict.fromkeys(df.sequence_id.tolist())):
        ...
        observed = df.loc[inds, "state"].tolist()
        formatted = observed[:]
        if collapse_repeats:
            formatted = [x for i, x in enumerate(formatted) if i == 0 or x != formatted[i - 1]]
        rows.append(
            {
                "sequence_id": sid,
                **_meta_values(df, inds, metadata),
                "n_observations": len(observed),
                "n_states": len(formatted),
                "n_unique_states": len(dict.fromkeys(formatted)),
                "start_state": formatted[0],
                "end_state": formatted[-1],
                "path": separator.join(formatted),
            }
        )
    return PathFormatResult(
        ...
        inp["status"],
        inp["mapping"],
        {"separator": separator, "collapse_repeats": collapse_repeats},
    )
