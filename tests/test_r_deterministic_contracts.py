...
def test_r_data_audit_empty_contract():
    audit = g.audit_sequence_data(valid_data(), "id", "position", "state", "duration_ms", ["participant"])
    assert list(audit.columns) == [
        "sequence_id", "row", "column", "issue_code", "severity", "value", "message", "action"
    ]
    assert audit.empty
...
def test_r_data_validation_normal_and_empty():
    ...
    assert result.valid and result.status == "pass" and result.n_sequences == 2 and result.n_rows == 6
...
def test_r_data_missing_columns_and_states():
    ...
    data = valid_data(); data.loc[1, "state"] = None
...
def test_r_data_order_gaps_and_duplicate_positions_audited():
    ...
    gap = valid_data(); gap.loc[2, "position"] = 4
...
def test_r_data_preparation_sort_and_identifiers():
    ...
    prepared = g.prepare_sequence_data(data, "id", "position", "state", "duration_ms", ["participant"])
    assert prepared.status == "pass"
    assert list(prepared.data.columns[:5]) == ["sequence_id", "sequence_order", "state", "original_row", "duration"]
...
def test_r_data_explicit_drop_policies():
    data = valid_data(); data.loc[1, "state"] = None; data.loc[4, "state"] = "unknown"
    prepared = g.prepare_sequence_data(
        data, "id", "position", "state",
        expected_states=["home", "search", "category", "product"],
        missing_state_policy="drop", unknown_state_policy="drop"
    )
...
def test_r_data_duplicate_and_repeat_policies():
    ...
    prepared = g.prepare_sequence_data(
        data, "id", "position", "state", "duration_ms",
        duplicate_position_policy="last", repeated_state_policy="collapse"
    )
    assert prepared.status == "review"
    ...
    out = prepared.audit
    assert ((out.stage == "output") & (out.issue_code == "missing_positions") & (out.severity == "review")).any()
...
def test_r_data_unresolved_errors_suppress_output():
    data = valid_data(); data.loc[1, "duration_ms"] = 0
    zero = g.prepare_sequence_data(data, "id", "position", "state", "duration_ms", zero_duration_policy="error")
...
def test_r_data_single_state_and_unused_levels_reviewable():
    data = pd.DataFrame({"id": ["s1"], "position": [1], "state": pd.Categorical(["A"], categories=["A", "B"])})
...
def test_r_encoding_deterministic_across_row_order():
    data = encoding_data(); shuffled = data.iloc[[6, 1, 4, 0, 3, 2, 5]].reset_index(drop=True)
    ...
    pd.testing.assert_frame_equal(first.data[cols].reset_index(drop=True), second.data[cols].reset_index(drop=True))


def test_r_encoding_custom_levels_and_labels():
    encoded = g.encode_sequence_data(encoding_data(), "id", "position", "state", state_levels=["C", "B", "A", "Z"], prefix="Q", width=2)
...
def test_r_encoding_categorical_levels_define_order():
    data = encoding_data(); data["state"] = pd.Categorical(data.state, categories=["C", "A", "B", "Z"])
...
def test_r_state_summary_exact_counts_and_proportions():
    result = g.summarise_sequence_states(encoding_data(), "id", "position", "state", "duration_ms", ["group"])
    ...
    assert s1a.duration_sum == pytest.approx(4) and s1a.duration_proportion == pytest.approx(.4)
    ...
    assert oa.duration_sum == pytest.approx(10) and oa.duration_proportion == pytest.approx(.5)
    assert oa.mean_duration == pytest.approx(10 / 3)


def test_r_state_summary_deterministic_across_input_order():
    data = encoding_data(); shuffled = data.iloc[[5, 0, 6, 3, 1, 4, 2]].reset_index(drop=True)
...
def test_r_transition_summary_exact_adjacent_counts():
    ...
    assert result.status == "pass"
    ab = result.by_sequence.query("sequence_id == 's1' and from_state == 'A' and to_state == 'B'").iloc[0]
    ...
    assert ba.n_sequences == 1 and ba.sequence_proportion == pytest.approx(.5)
    assert ba.n_transitions == 1 and ba.transition_proportion == pytest.approx(.2)
    assert ba.origin_transition_proportion == pytest.approx(.5)
...
def test_r_transition_single_state_empty_schema():
    result = g.summarise_sequence_transitions(pd.DataFrame({"id": ["s1"], "position": [1], "state": ["A"]}), "id", "position", "state")
    assert result.status == "review" and result.by_sequence.empty and result.overall.empty
    assert list(result.overall.columns) == ["from_state", "to_state", "n_sequences", "sequence_proportion", "n_transitions", "transition_proportion", "origin_transition_proportion"]


def test_r_paths_ordered_and_metadata_retained():
    data = encoding_data(); shuffled = data.iloc[[6, 3, 1, 5, 0, 4, 2]].reset_index(drop=True)
    result = g.format_sequence_paths(shuffled, "id", "position", "state", ["group"])
    assert result.status == "review"
    assert ((result.audit.issue_code == "unordered_rows") & (result.audit.severity == "review")).any()
...
def test_r_paths_collapse_consecutive_repeats():
    data = pd.DataFrame({"id": ["s1"] * 5, "position": range(1, 6), "state": ["A", "A", "B", "B", "C"]})
    result = g.format_sequence_paths(data, "id", "position", "state", separator="/", collapse_repeats=True)
    assert result.status == "review"
...
def test_r_summary_functions_reject_unresolved_validation_errors():
    data = encoding_data(); data.loc[1, "position"] = 1
    for func in [g.encode_sequence_data, g.summarise_sequence_states, g.summarise_sequence_transitions, g.format_sequence_paths]:
...
def test_r_motifs_contiguous_stable_positions():
    result = g.extract_sequence_ngrams(motif_data(), "id", "position", "state", metadata_cols=["group"], min_length=2, max_length=3, overlap="allow")
    ...
    assert np.all(result.occurrences.end_index - result.occurrences.start_index + 1 == result.occurrences.motif_length)
    assert result.occurrences.loc[result.occurrences.sequence_id == "s1", "group"].tolist() == ["g1"] * 7
...
def test_r_motifs_overlap_policy_deterministic():
    allowed = g.extract_sequence_ngrams(motif_data(), "id", "position", "state", min_length=3, max_length=3, overlap="allow")
    disallowed = g.extract_sequence_ngrams(motif_data(), "id", "position", "state", min_length=3, max_length=3, overlap="disallow")
    ...
    rep = g.extract_sequence_ngrams(repeated, "id", "position", "state", min_length=2, max_length=2, overlap="disallow")
    assert rep.status == "review" and rep.occurrences.start_index.tolist() == [1, 3]


def test_r_motifs_row_order_corrected_with_review():
    data = motif_data(); shuffled = data.iloc[[6, 1, 8, 0, 5, 3, 2, 7, 4]].reset_index(drop=True)
    ordered = g.extract_sequence_ngrams(data, "id", "position", "state", min_length=2, max_length=3)
    reordered = g.extract_sequence_ngrams(shuffled, "id", "position", "state", min_length=2, max_length=3)
    assert ordered.status == "pass" and reordered.status == "review"
    assert "unordered_rows" in set(reordered.audit.issue_code)
    cols = ["sequence_id", "motif_id", "motif", "motif_length", "start_index", "end_index", "start_order", "end_order", "occurrence_index"]
    pd.testing.assert_frame_equal(ordered.occurrences[cols].reset_index(drop=True), reordered.occurrences[cols].reset_index(drop=True))


def test_r_motifs_separator_labels_keep_identity():
    data = pd.DataFrame({"id": ["single", "pair", "pair"], "position": [1, 1, 2], "state": ["A > B", "A", "B"]})
    result = g.extract_sequence_ngrams(data, "id", "position", "state", min_length=1, max_length=2, separator=" > ")
    same = result.occurrences.loc[result.occurrences.motif == "A > B", ["motif_id", "motif_key", "motif_length"]]
...
def test_r_motif_summary_exact_counts_prevalence():
    summary = g.summarise_sequence_motifs(g.extract_sequence_ngrams(motif_data(), "id", "position", "state", min_length=2, max_length=3))
    ...
    assert aba.sequence_prevalence == pytest.approx(1) and aba.occurrence_share == pytest.approx(3/12)
...
def test_r_motif_prevalence_denominator_includes_short_sequences():
    ...
    extracted = g.extract_sequence_ngrams(data, "id", "position", "state", metadata_cols=["group"], min_length=3, max_length=3)
...
def test_r_motif_filter_thresholds():
    summary = g.summarise_sequence_motifs(g.extract_sequence_ngrams(motif_data(), "id", "position", "state", min_length=2, max_length=3))
    filtered = g.filter_sequence_motifs(summary, min_occurrences=3, min_sequences=2, min_prevalence=1, motif_lengths=[2])
...
def test_r_motif_format_units_and_rank_ties():
    summary = g.summarise_sequence_motifs(g.extract_sequence_ngrams(motif_data(), "id", "position", "state", min_length=2, max_length=3))
    formatted = g.format_sequence_motifs(summary, prevalence="percent", digits=1, rank_by="n_occurrences", ties="min")
    assert "sequence_prevalence_percent" in formatted.table and "occurrence_share_percent" in formatted.table
...
def test_r_motif_empty_outputs_stable_schemas():
    extracted = g.extract_sequence_ngrams(motif_data(), "id", "position", "state", min_length=6, max_length=6)
    summary = g.summarise_sequence_motifs(extracted)
    filtered = g.filter_sequence_motifs(summary, min_occurrences=2)
    formatted = g.format_sequence_motifs(filtered)
    assert extracted.occurrences.empty and extracted.motifs.empty and summary.by_sequence.empty and summary.overall.empty
...
def test_r_motif_categorical_levels_define_codes():
    data = motif_data(); data["state"] = pd.Categorical(data.state, categories=["C", "A", "B", "Z"])
    ...
    assert not bool(result.state_dictionary.loc[result.state_dictionary.state == "Z", "observed"].iloc[0])
...
def test_r_motif_invalid_settings_rejected():
    ...
    duplicated = data.copy(); duplicated.loc[1, "position"] = 1
    with pytest.raises(ValidationError, match="failed validation"):
...
def test_r_metamorphic_distance_invariant_to_row_order():
    data = minimal_case(); shuffled = data.sample(frac=1, random_state=2026).reset_index(drop=True)
...
def test_r_metamorphic_irrelevant_metadata_no_distance_change():
    data = minimal_case(); other = data.copy(); other["irrelevant_metadata"] = [f"row{i}" for i in range(len(other))]
    a = g.compute_sequence_distance(data, method="lcs"); b = g.compute_sequence_distance(other, method="lcs")
    assert np.array_equal(a.matrix, b.matrix)


def test_r_metamorphic_state_relabelling_preserves_levenshtein_geometry():
    data = minimal_case(); other = data.copy(); mapping = {"A":"north","B":"south","C":"east","D":"west"}; other["state"] = other.state.map(mapping)
    a = g.compute_sequence_distance(data, method="levenshtein"); b = g.compute_sequence_distance(other, method="levenshtein")
    assert np.array_equal(a.matrix, b.matrix)
...
def test_r_adversarial_failures_are_explicit():
    ...
    dup = base.copy(); dup.loc[1, "sequence_order"] = dup.loc[0, "sequence_order"]; cases.append(dup)
    neg = base.copy(); neg.loc[1, "duration"] = -1; cases.append(neg)
    for data in cases:
        result = g.validate_sequence_data(data, "sequence_id", "sequence_order", "state", "duration")
        assert result.status == "fail"


def test_r_adversarial_review_cases_not_silently_destroyed():
    base = minimal_case()
    gaps = base.copy(); gaps.loc[gaps.sequence_id == "s1", "sequence_order"] = [1,2,4,5]
    zero = base.copy(); zero.loc[1, "duration"] = 0
    rep = base.copy(); rep["state"] = np.tile(["A","A","A","B"],4)
