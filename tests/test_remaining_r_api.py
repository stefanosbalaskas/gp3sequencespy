...
def data8():
    ...
    for i,(sid,path) in enumerate(zip(ids,paths,strict=True)):
        ...
        for pos,state in enumerate(path,1):
            rows.append({"sequence_id":sid,"sequence_order":pos,"state":state,"group":group,"participant_id":f"p{i+1}"})
    return pd.DataFrame(rows)


def test_frozen_81_function_manifest_is_present():
    manifest=json.loads((Path(__file__).parents[1]/"reference"/"api_manifest.json").read_text())
...
def test_gp3tools_mapping_and_ambiguity_guards():
    ...
    ambiguous=data8(); ambiguous["trial_id"]=ambiguous.sequence_id
    ...
    durations=data8(); durations["fixation_duration"]=100; durations["event_duration"]=100
...
def test_capabilities_are_deterministic_and_do_not_import_optional_modules():
    ...
    assert {"family","capability","role","native","backend","backend_required","available","installed_version","minimum_tested_version","reference_only","notes"}.issubset(first.columns)
...
def test_extended_plots_return_axes_with_plotted_data():
    ...
    ax=g.plot_consensus_sequence(consensus,group="g1"); assert len(ax.gp3_data)==5; plt.close(ax.figure)
    ax=g.plot_consensus_sequence(g.create_consensus_sequence(data),type="states"); assert len(ax.gp3_data)==5; plt.close(ax.figure)
    comparison=g.compare_sequence_groups(data,"group")
    ax=g.plot_sequence_group_comparison(comparison,"state",top_n=4); assert len(ax.gp3_data)>0; plt.close(ax.figure)
    ax=g.plot_sequence_group_comparison(comparison,"length"); assert len(ax.gp3_data)==2; plt.close(ax.figure)
    ax=g.plot_sequence_index(data); assert ax.gp3_data.shape==(8,5); plt.close(ax.figure)
    ax=g.plot_sequence_state_distribution(data); assert ax.gp3_data.shape==(5,4); assert np.allclose(ax.gp3_data.sum(axis=1),1); plt.close(ax.figure)
    ax=g.plot_sequence_entropy(data); assert len(ax.gp3_data)==5; assert ax.gp3_data.entropy.between(0,1).all(); plt.close(ax.figure)
    distance=g.compute_sequence_distance(data)
    clustering=g.cluster_sequences(distance,2)
    ax=g.plot_sequence_distance_heatmap(distance,order_by=clustering); assert ax.gp3_data.shape==(8,8); plt.close(ax.figure)
    network=g.create_transition_network(data,normalise="from")
    ax=g.plot_transition_network(network); assert len(ax.gp3_data)>0; plt.close(ax.figure)
    ax=g.plot_sequence_cluster_silhouette(clustering,distance); assert len(ax.gp3_data)==8; plt.close(ax.figure)
