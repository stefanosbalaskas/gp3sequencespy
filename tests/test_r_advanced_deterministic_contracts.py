...
def test_r_consensus_creation_deterministic_support():
    ...
    assert len(first) == 10 and (first.support_n == 4).all() and first.agreement.between(.5, 1).all()
    pd.testing.assert_frame_equal(first, second)


def test_r_consensus_tie_and_missing_state_policies():
    data = pd.DataFrame({"sequence_id": ["s1", "s2"], "sequence_order": [1, 1], "state": ["B", "A"]})
    assert g.create_consensus_sequence(data, tie_method="first", state_levels=["A", "B"]).iloc[0].consensus_state == "A"
    assert g.create_consensus_sequence(data, tie_method="last", state_levels=["A", "B"]).iloc[0].consensus_state == "B"
    assert pd.isna(g.create_consensus_sequence(data, tie_method="missing", state_levels=["A", "B"]).iloc[0].consensus_state)
    assert g.create_consensus_sequence(data, tie_method="all", state_levels=["A", "B"]).iloc[0].consensus_state == "A | B"
...
def test_r_group_comparison_expected_components():
    ...
    assert {"state", "event_share", "sequence_prevalence"}.issubset(comparison.state_summary.columns)
...
def test_r_group_comparison_rejects_incomplete_metadata():
    data = advanced_data(); data.loc[data.sequence_id == "s01", "group"] = None
...
def test_r_zero_weight_does_not_inflate_support():
    data = pd.DataFrame({"sequence_id": ["s1", "s2"], "sequence_order": [1, 1], "state": ["A", "B"], "weight": [1, 0]})
...
def test_r_group_transition_separator_guard():
    data = advanced_data(); data.loc[data.state == "A", "state"] = "A -> embedded"
...
def test_r_state_plot_preserves_unresolved_tie():
    data = pd.DataFrame({"sequence_id": ["s1", "s2"], "sequence_order": [1, 1], "state": ["A", "B"]})
...
def test_r_empty_transition_comparison_plot_fails_clearly():
    data = pd.DataFrame({"sequence_id": ["s1", "s2"], "sequence_order": [1, 1], "state": ["A", "B"], "group": ["g1", "g2"]})
...
def test_r_distance_normalisation_and_substitution_matrix_guard():
    data = advanced_data(); states = sorted(data.state.unique())
    costs = pd.DataFrame(2.0, index=states, columns=states)
    for state in states:
        costs.loc[state, state] = 0.0
    raw = g.compute_sequence_distance(data, method="optimal_matching", substitution_matrix=costs, normalise="none")
    norm = g.compute_sequence_distance(data, method="optimal_matching", substitution_matrix=costs, normalise="max_length")
    assert (norm.matrix <= raw.matrix + 1e-12).all()
    with pytest.raises(ValidationError):
        g.compute_sequence_distance(data, method="optimal_matching", substitution_matrix=np.ones((2, 3)))
...
def test_r_bootstrap_cluster_stability_reproducible():
    distance = g.compute_sequence_distance(advanced_data(), method="lcs")
    a = g.bootstrap_sequence_clusters(distance, 2, n_boot=8, sample_fraction=.75, seed=41)
    b = g.bootstrap_sequence_clusters(distance, 2, n_boot=8, sample_fraction=.75, seed=41)
...
def test_r_cluster_ensemble_coassociation_contract():
    ...
    c1 = g.cluster_sequences(d1, 2); c2 = g.cluster_sequences(d2, 2)
...
def test_r_stochastic_cluster_helpers_do_not_mutate_global_numpy_rng():
    ...
    g.bootstrap_sequence_clusters(distance, 2, n_boot=3, sample_fraction=.75, seed=9)
...
def test_r_unused_categorical_levels_not_observed_distance_states():
    data = advanced_data(); data["state"] = pd.Categorical(data.state, categories=["A","B","C","D","UNUSED"])
    states = ["A","B","C","D"]
    costs = pd.DataFrame(1.0, index=states, columns=states)
    for state in states:
        costs.loc[state, state] = 0.0
    distance = g.compute_sequence_distance(data, method="optimal_matching", substitution_matrix=costs)
    assert "UNUSED" not in set(distance.sequences.state.astype(str))


def test_r_ensemble_linkage_and_stability_validation():
    data = advanced_data(); d1 = g.compute_sequence_distance(data, method="lcs"); d2 = g.compute_sequence_distance(data, method="transition")
    c1 = g.cluster_sequences(d1, 2); c2 = g.cluster_sequences(d2, 2)
    with pytest.raises(ValidationError, match="not a supported"):
        g.create_sequence_cluster_ensemble(c1, c2, k=2, linkage="unknown")
    boot = g.bootstrap_sequence_clusters(d1, 2, n_boot=2, sample_fraction=.5, seed=4)
...
def test_r_clara_repeatable_and_global_rng_safe():
    distance = g.compute_sequence_distance(advanced_data(), method="lcs")
    np.random.seed(321); before = np.random.get_state()
...
def test_r_integer_seed_controls_reject_invalid_values_and_boundary_safe():
    ...
    result = g.bootstrap_sequence_clusters(distance, 2, n_boot=1, sample_fraction=.75, seed=2**31-1)
...
def test_r_first_order_network_edge_measures():
    data = advanced_data(); network = g.create_transition_network(data, normalise="from")
    assert {"from_state","to_state","count","weight","sequence_count","sequence_prevalence"}.issubset(network.columns)
...
def test_r_higher_order_network_and_model_contexts():
    data = advanced_data(); network = g.create_transition_network(data, order=2, normalise="from")
    assert network.from_state.isna().all() and network.context.str.contains(" > ", regex=False).all()
    model = g.fit_higher_order_transition_model(data, order=3, smoothing=.5, backoff=True)
...
def test_r_network_centrality_and_communities_deterministic():
    ...
    assert {"state","total_degree","total_strength","closeness","betweenness","pagerank"}.issubset(centrality.columns)
    assert centrality.pagerank.sum() == pytest.approx(1, abs=1e-8)
    a = g.detect_transition_communities(network, seed=9); b = g.detect_transition_communities(network, seed=9)
...
def test_r_network_bootstrap_reproducible_bounded():
    data = advanced_data(); a = g.bootstrap_transition_network(data, n_boot=10, seed=17); b = g.bootstrap_transition_network(data, n_boot=10, seed=17)
    pd.testing.assert_frame_equal(a,b)
...
def test_r_unseen_context_probability_schema():
    ...
    assert {"order","context","next_state","count","probability","used_order","used_context"}.issubset(unseen.columns)
    assert (unseen.used_order == 0).all() and len(unseen) == 2


def test_r_network_bootstrap_global_rng_safe():
    np.random.seed(901); before = np.random.get_state()
