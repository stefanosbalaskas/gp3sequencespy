...
def test_motif_contracts():
    e = extract_sequence_ngrams(
        motifdata(), "id", "position", "state", metadata_cols=["group"], min_length=2, max_length=3
    )
    ...
    dis = extract_sequence_ngrams(
        motifdata(), "id", "position", "state", min_length=3, max_length=3, overlap="disallow"
    )
...
def _advanced_data():
    ...
    for i, (sid, path) in enumerate(paths.items(), 1):
        ...
        rows += [
            ...
            for j, s in enumerate(path, 1)
        ]
    return pd.DataFrame(rows)


def test_consensus_and_group_comparison_contracts():
    from gp3sequencespy.consensus import (
    ...
    assert con["agreement"].between(0.5, 1).all()
...
def test_consensus_ties_and_zero_weights():
    ...
    assert (
        create_consensus_sequence(d, tie_method="first", state_levels=["A", "B"])
        .iloc[0]
        .consensus_state
        == "A"
    )
    assert (
        create_consensus_sequence(d, tie_method="last", state_levels=["A", "B"])
        .iloc[0]
        .consensus_state
        == "B"
    )
    assert pd.isna(
        create_consensus_sequence(d, tie_method="missing", state_levels=["A", "B"])
        .iloc[0]
        .consensus_state
    )
    assert (
        create_consensus_sequence(d, tie_method="all", state_levels=["A", "B"])
        .iloc[0]
        .consensus_state
        == "A | B"
    )
...
def test_distance_normalisation_and_bootstrap_reproducible():
    from gp3sequencespy import bootstrap_sequence_clusters, compute_sequence_distance

    d = _advanced_data()
    a = compute_sequence_distance(d, method="lcs")
    b = compute_sequence_distance(d, method="lcs", normalise="max_length")
    ...
    assert np.allclose(
        x.pairwise_stability.to_numpy(), y.pairwise_stability.to_numpy(), equal_nan=True
    )
...
def test_transition_network_markov_and_bootstrap():
    ...
    d = _advanced_data()
    n = create_transition_network(d, normalise="from")
    assert np.allclose(n.groupby("context").weight.sum().to_numpy(), 1)
    ns = create_transition_network(d, include_self=False)
    assert not (ns.from_state == ns.to_state).any()
    n2 = create_transition_network(d, order=2, normalise="from")
    assert n2.from_state.isna().all() and n2.context.str.contains(" > ", regex=False).all()
    c = summarise_transition_centrality(create_transition_network(d, normalise="count"))
    assert np.isclose(c.pagerank.sum(), 1)
    assert detect_transition_communities(n, seed=9).equals(detect_transition_communities(n, seed=9))
    m = fit_higher_order_transition_model(d, order=3, smoothing=0.5, backoff=True)
    p = predict_next_state(m, ["A", "B", "C"])
    assert np.isclose(p.probability.sum(), 1) and p.used_order.iloc[0] <= 3
    u = predict_next_state(m, ["Z"])
    assert (u.used_order == 0).all()
    b1 = bootstrap_transition_network(d, n_boot=5, seed=17)
    b2 = bootstrap_transition_network(d, n_boot=5, seed=17)
...
def test_panel_workflow():
    ...
    for p in range(1, 5):
        for occ in [1, 2]:
            sid = f"p{p}_w{occ}"
            path = ["A", "B", "C"] if occ == 1 else ["A", "C", "C"]
            rows += [
                {
    ...
    d = pd.DataFrame(rows)
    panel = prepare_sequence_panel(d, "participant_id", "occasion")
    sm = summarise_sequence_panel(panel)
    assert len(panel.index) == 8 and sm["n_panels"] == 4 and sm["n_occasions"] == 2
    ch = compare_sequence_panel_changes(panel, method="lcs")
    assert len(ch) == 4 and (ch.distance >= 0).all()
...
def test_noncontiguous_subsequences():
    ...
    d = _advanced_data()
    o = extract_sequence_subsequences(
        d, metadata_cols="group", min_length=2, max_length=3, max_gap=2, max_span=4
    )
    assert (
        len(o) > 0
        and o.subsequence_length.isin([2, 3]).all()
        and (o.max_observed_gap <= 2).all()
        and (o.span <= 4).all()
    )
    s = summarise_sequence_subsequences(o)
    assert s.sequence_prevalence.between(0, 1).all()
    f = filter_sequence_subsequences(s, min_sequences=2, top_n=5, ties="exclude")
    assert len(f) <= 5
    c = compare_sequence_subsequences(o, "group")
    assert {"p_value", "p_adjusted"}.issubset(c.columns) and (
        c.p_adjusted >= c.p_value - 1e-12
    ).all()
...
def test_hmm_core_and_mixture():
    ...
    d = _advanced_data()
    m = fit_sequence_hmm(d, 2, max_iter=20, seed=11)
    assert (
        np.isclose(m.initial_probs.sum(), 1)
        and np.allclose(m.transition_probs.sum(1), 1)
        and np.allclose(m.emission_probs.sum(1), 1)
        and np.isfinite(m.log_likelihood)
    )
    dec = decode_sequence_states(m)
    assert len(dec) == len(d) and dec.posterior_probability.between(0, 1).all()
    m3 = fit_sequence_hmm(d, 3, max_iter=10, seed=4)
    cmp = compare_sequence_hmms(two=m, three=m3)
    assert len(cmp) == 2 and {"delta_aic", "delta_bic", "converged"}.issubset(cmp.columns)
    mix = fit_sequence_hmm_mixture(d, 2, 2, max_iter=8, inner_initial_iter=3, seed=21)
    probs = mix.responsibilities[["component_1", "component_2"]].to_numpy()
    assert np.allclose(probs.sum(1), 1)
    md = decode_sequence_states(mix)
    assert len(md) == len(d) and set(md.component).issubset({1, 2})
...
def test_design_aware_inference():
    ...
    d = _advanced_data()
    design = declare_sequence_comparison_design("group", "participant_id", "observational")
    inf = test_sequence_group_difference(
        d, design, metric="state_prevalence", target_state="A", n_permutations=49, seed=4
    )
    assert (
        len(inf.unit_data) == 8
    ...
    inf = bootstrap_sequence_group_difference(inf, n_boot=49, seed=5)
    sm = summarise_sequence_group_inference(inf)
    assert len(sm["bootstrap_interval"]) == 1
...
def test_multichannel_hmm():
    ...
    d = _advanced_data().copy()
    d["channel_context"] = np.tile(["x", "x", "y", "y", "z"], 8)
    m = fit_multichannel_sequence_hmm(d, 2, ["state", "channel_context"], max_iter=4, seed=11)
    assert (
        m.transition_probs.shape == (2, 2)
        and len(m.emission_probs) == 2
        and np.isfinite(m.log_likelihood)
    )
    dec = decode_multichannel_sequence_states(m)
    assert len(dec) == len(d)
    sm = summarise_multichannel_sequence_hmm(m)
    assert {"fit", "initial", "transition", "emission"} <= sm.keys()
...
def test_time_varying_model_contract():
    ...
    for i in range(24):
        ...
        for t in range(1, 13):
            lp = -0.4 + 0.06 * t + 0.35 * (group == "g2") * np.sin(t / 3)
            rows.append(
                {
    ...
    fit = g.fit_time_varying_sequence_model(
        d, "group", "participant_id", target_state="A", k=3, include_random_effect=False
    )
    pred = g.predict_time_varying_sequence_model(fit, time=[1, 6, 12])
    assert len(pred) == 6 and pred.estimate.between(0, 1).all()
    sm = g.summarise_time_varying_sequence_model(fit)
    assert {"metadata", "smooth_terms", "converged"} <= set(sm)
    ax = g.plot_time_varying_sequence_model(fit, time=[1, 6, 12])
...
def test_motif_position_and_plot_contracts():
    ...
    ex = g.extract_sequence_ngrams(
        d, "id", "position", "state", metadata_cols=["group"], min_length=2, max_length=3
    )
    p = g.summarise_sequence_motif_positions(ex, "start", "absolute")
    ab = p.summary.loc[p.summary.motif == "A > B"].iloc[0]
    assert ab.n_occurrences == 4 and ab.n_sequences == 3 and ab.mean_position == 1.75
    pr = g.summarise_sequence_motif_positions(ex, "start", "relative", by="group")
    fmt = g.format_sequence_motif_positions(pr, digits=1, position_units="percent")
    assert "rank" in fmt["table"] and set(fmt["table"].position_unit) == {"percent"}
    ax = g.plot_sequence_motifs(
        g.summarise_sequence_motifs(ex), metric="n_occurrences", top_n=3, ties="first"
    )
    assert len(ax.gp3_data) == 3
    ax2 = g.plot_sequence_motif_positions(ex, position="centre", scale="relative", top_n=2)
    assert len(ax2.gp3_motif_table) == 2 and ax2.gp3_data.position_value.between(0, 1).all()
