...
- Total current Python tests (including bootstrap/public-API/compatibility and documentation-contract tests): **162**
- Current validated result before the docs-site commit: **158 passed**; the documentation tranche adds 4 contract tests for a **162-test** full suite.
- Executable R oracle in this environment: **not available** (no R executable in the current container)
...
| 49 | `test-sequence-distances-clustering.R #4: hierarchical clustering and validation are auditable` | `tests/test_r_advanced_deterministic_contracts.py::test_r_hierarchical_cluster_validation_and_representatives` | `semantic_contract; R_clustering_oracle_pending` |
| 50 | `test-sequence-distances-clustering.R #5: PAM is guarded by the optional cluster package` | `tests/test_r_advanced_deterministic_contracts.py::test_r_pam_native_backend_available` | `semantic_contract; R_clustering_oracle_pending` |
| 51 | `test-sequence-distances-clustering.R #6: bootstrap stability is reproducible` | `tests/test_r_advanced_deterministic_contracts.py::test_r_bootstrap_cluster_stability_reproducible` | `semantic_contract; R_clustering_oracle_pending` |
| 52 | `test-sequence-distances-clustering.R #7: cluster ensembles use transparent co-association` | `tests/test_r_advanced_deterministic_contracts.py::test_r_cluster_ensemble_coassociation_contract` | `semantic_contract; R_clustering_oracle_pending` |
...
| 54 | `test-sequence-distances-clustering.R #9: CLARA is guarded and preserves sequence identifiers` | `tests/test_r_advanced_deterministic_contracts.py::test_r_clara_preserves_sequence_identifiers` | `semantic_contract; R_clustering_oracle_pending` |
...
| 58 | `test-sequence-distances-clustering.R #13: ensemble linkage and stability summaries are validated` | `tests/test_r_advanced_deterministic_contracts.py::test_r_ensemble_linkage_and_stability_validation` | `semantic_contract; R_clustering_oracle_pending` |
| 59 | `test-sequence-distances-clustering.R #14: optional CLARA clustering restores the caller RNG and is repeatable` | `tests/test_r_advanced_deterministic_contracts.py::test_r_clara_repeatable_and_global_rng_safe` | `semantic_contract; R_clustering_oracle_pending` |
